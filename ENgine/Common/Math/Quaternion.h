...
public:
	...
	Matrix& GetMatrix(Matrix & mtx) const;
	...
	void Rotate2( const Vector& dir, const Vector& dest );
};
...
inline Matrix& Quaternion::GetMatrix(Matrix& mtx) const
{
...
inline void Quaternion::Rotate2( const Vector& dir, const Vector& dest )
{
	float d = dest | dir;
	...
	Vector normal = dest ^ dir;
	
	normal.Normalize();
	float angle = dir.GetAngle(dest);
