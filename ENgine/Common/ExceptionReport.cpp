...
//
TCHAR ExceptionReport::m_szPDBSearchPath[MAX_PATH];
TCHAR ExceptionReport::m_szLogFileName[MAX_PATH];
TCHAR ExceptionReport::m_szLogFileNameDump[MAX_PATH];


LPTOP_LEVEL_EXCEPTION_FILTER ExceptionReport::m_previousFilter;
HANDLE ExceptionReport::m_hReportFile;
HANDLE ExceptionReport::m_hProcess;
...
{
	char curDirTemp[MAX_PATH];
	GetCurrentDirectory(MAX_PATH-2, curDirTemp);
	
	sprintf(m_szPDBSearchPath, "%s\\debuginfo\\", curDirTemp);
	sprintf(m_szLogFileNameDump, "%s\\crush_dump.dmp", curDirTemp);
	sprintf(m_szLogFileName, "%s\\crash.txt", curDirTemp);
}
...
//==============================================================
void ExceptionReport::SetLogFileName( const char * pszLogFileName )
{
	...
	sprintf(tmp, "%s\\CrashLogs\\%i_%i_%i_%i_%i_%i\\", curDirTemp, GetTime(5),GetTime(4),GetTime(3),GetTime(0),GetTime(1),GetTime(2));
	CreateDirectories(tmp);

	sprintf(m_szLogFileNameDump, "%s\\%s_dump.dmp", tmp, pszLogFileName);
	sprintf(m_szLogFileName, "%s\\%s.txt", tmp, pszLogFileName);
}

LONG ExceptionReport::WriteMiniDump( struct _EXCEPTION_POINTERS *pExceptionInfo )
{
...
//===========================================================
LONG WINAPI ExceptionReport::WheatyUnhandledExceptionFilter(
                                    PEXCEPTION_POINTERS pExceptionInfo )
{
	SetLogFileName("Crash");
...
//===========================================================================
void ExceptionReport::GenerateExceptionReport(
    PEXCEPTION_POINTERS pExceptionInfo )
{
...
//=============================================================================
BOOL ExceptionReport::GetLogicalAddress(
...
//============================================================
void ExceptionReport::WriteStackDetails(
        PCONTEXT pContext,
        ...
        bool & bHandled )
{
    ...
    {
        // Add appropriate indentation level (since this rou\tine is recursive)
