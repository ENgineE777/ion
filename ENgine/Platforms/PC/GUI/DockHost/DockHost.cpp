...
/*
# include <LogWindow.h>
extern CLogWindow g_Log;



VOID DockHost_PrintNode( HWND hDockHost, CDHNode* pNode )
{
	g_Log( TEXT( "node %p" ), pNode );
	if( !pNode )
		return;
	if( pNode->GetType() == DH_LEAF )
	{
		...
		if( hWnd )
		{
			...
			g_Log( TEXT( "Window: %s" ), s );
		}
	}
	g_Log( TEXT( "Type: %d" ), pNode->GetType() );
	RECT rNode;
	DockHost_GetNodeRect( hDockHost, CONV(pNode), &rNode );
	g_Log( TEXT( "rectangle: (%d %d %d %d)" ), rNode.left, rNode.top, rNode.right, rNode.bottom );
	g_Log( TEXT( "minSize: (%d %d)" ), pNode->GetMinSize().cx, pNode->GetMinSize().cy );
	g_Log( TEXT( "resizability: %d" ), pNode->GetResizability() );
	CDHDividerNode* pDivider = GetDividerNode( pNode );
	if( pDivider )
	{
		g_Log( TEXT( "divider position: %d" ), pDivider->pos );
		g_Log( TEXT( "Child A Infos:" ) );
		g_Log.Indent();
		DockHost_PrintNode( hDockHost, pDivider->ChildA() );
		g_Log.Unindent();
		g_Log( TEXT( "Child B infos:" ) );
		g_Log.Indent();
		DockHost_PrintNode( hDockHost, pDivider->ChildB() );
		g_Log.Unindent();
	}
}

VOID DockHost_PrintInfo( HWND hDockHost )
{
	g_Log.Clear();
	DockHost_PrintNode( hDockHost, CONV( DockHost_GetRootNode( hDockHost ) ) );
}
*/
...
BOOL DockHost_GetLeafNodeRects( HWND hDockHost, HDHNODE hNode,RECT* prNode,RECT* prClient,RECT* prTab,RECT* prCaptionBar,RECT* prClose,RECT* prText,RECT* prIcon )
{
	...
	{
		CalculateDockedCaptionRects( DockHost_GetNodeWindow( hNode ), rNode, &rCaptionBar,prText, prIcon, prClose,(DockHost_GetNodeStyle( hNode ) & DHNS_NO_CLOSE) == 0);
	}
	else
	{
...
// between both sides.
//  lengthA -> INOUT - at entry, it's the current length of A.
//  lengthB -> INOUT - at entry, it's the current length of B.
VOID DockHost_DivideSpace( INT length,INT minLengthA,INT minLengthB,INT resizabilityA,INT resizabilityB,LONG& lengthA,LONG& lengthB)
{
...
VOID DockHost_OnNodeResized( HWND hDockHost, HDHNODE hNode )
{
	...
	DockHost_DivideSpace( newSize.cx - dividerWidth,pDivider->ChildA()->GetMinSize().cx,pDivider->ChildB()->GetMinSize().cx,pDivider->ChildA()->GetResizability(),pDivider->ChildB()->GetResizability(),sizeA.cx, sizeB.cx );

	// Calculate the new height values for both sides.
	DockHost_DivideSpace( newSize.cy - dividerHeight,pDivider->ChildA()->GetMinSize().cy,pDivider->ChildB()->GetMinSize().cy,pDivider->ChildA()->GetResizability(),	pDivider->ChildB()->GetResizability(),sizeA.cy, sizeB.cy );
...
VOID DockHost_SetNodeWindow( HWND hDockHost, HDHNODE hNode, HWND hWnd )
{
	...
	if( pNode->hWnd )
	{
		...
		ModifyWindowStyle( pNode->hWnd,WS_CHILD,/*WS_POPUP | WS_SYSMENU | WS_THICKFRAME | WS_CAPTION );*/pNode->dwUndockedStyle );

		SetParent( pNode->hWnd, NULL );
		SetWindowLongPtr( pNode->hWnd, GWLP_HWNDPARENT, (LONG)hOwner );
        RECT rNode;
        DockHost_GetNodeRect( hDockHost, hNode, &rNode );
        int newHeigth = RectHeight( rNode );
        int newWidth =  RectWidth( rNode );
        if(newHeigth < pNode->minSize.cy)
            newHeigth = pNode->minSize.cy;
        if(newWidth < pNode->minSize.cx)
            newWidth = pNode->minSize.cx;

		MoveWindow( pNode->hWnd, pNode->undockedRect.left, pNode->undockedRect.top,newWidth,newHeigth , TRUE );
	}
	pNode->hWnd = hWnd;
	if( hWnd )
	{
		...
		ModifyWindowStyle( hWnd,WS_OVERLAPPED | WS_SYSMENU |  WS_CAPTION | WS_THICKFRAME,WS_CHILD );
...
VOID DockHost_InsertNode( HWND hDockHost, DHNode* pNode, DHNode* pSibling,DH_DockLocation dockLocation, INT newItemWidth, INT newItemHeight )
{
...
// Internal function used by DockHost_Undock() and DockHost_TransferNodes().
// The function removes a node (leaf or divider) from the tree. If the node has a parent divider, the
// divider is deleted to maintain tree integrity.
// The function just removes the specified node from the node tree. The removed node and its children
// are not deleted, and the windows associated with them are not undocked.
...
HDHNODE DockHost_Dock( HWND hDockHost, HWND hWndToDock, DH_DockLocation dockLocation,HDHNODE hBaseNode, DWORD dwNodeStyle,INT newItemWidth, INT newItemHeight,INT newItemMinWidth, INT newItemMinHeight, INT resizability,VOID* userData )
{
...
BOOL DockHost_TransferNodes( HWND hSrcDockHost, HDHNODE hSrcNode,HWND hDockHost,DH_DockLocation dockLocation,HDHNODE hBaseNode )
{
	if( DockHost_IsDockLocationRelative( dockLocation ) && DockHost_IsEmpty( hDockHost ) )
		dockLocation = DH_LEFT;
	...
	RECT rNode;
	DockHost_GetNodeRect( hSrcDockHost, hSrcNode, &rNode );

	DHNode* pNode = CONV( hSrcNode );

	INT newItemMinWidth = pNode->GetMinSize().cx;
	INT newItemMinHeight = pNode->GetMinSize().cy;
	INT newItemWidth = RectWidth( rNode );
	INT newItemHeight = RectHeight( rNode );
	...
	CONV(hDockHost)->pLastHitNode = NULL;
	CONV(hSrcDockHost)->pLastHitNode = NULL;

	DockHost_RemoveNode( hSrcDockHost, pNode );
	DockHost_InsertNode( hDockHost, pNode, CONV(hSibling), dockLocation, newItemWidth, newItemHeight );

	for( HDHNODE hNode = hSrcNode; hNode != NULL; hNode = DockHost_GetNextNode( hDockHost, hNode ) )
	{
		if( DockHost_GetNodeType( hNode ) != DH_LEAF )
			continue;
		...
		DockHost_SetNodeWindow( hDockHost, hNode, hWnd );
		ShowWindow( hWnd, SW_SHOW );
	}
	DockHost_OnNodeResized( hDockHost, hSrcNode );
	InvalidateRect( hDockHost, NULL, FALSE );
	UpdateWindow( hDockHost );
	UpdateWindow( hSrcDockHost );
...
// Finds the node with the specified window.
HDHNODE DockHost_FindNodeByHWND( HWND hDockHost, HWND hWnd )
{
	for( HDHNODE hNode = DockHost_GetRootNode( hDockHost );
		hNode != NULL;
		hNode = DockHost_GetNextNode( hDockHost, hNode ) )
	{
		// TODO: Implement searching through leaf nodes which have more than one window.
		if( DockHost_GetNodeWindow( hNode ) == hWnd )
			return hNode;
	}
	return NULL;
}
...
VOID PaintLeafNode( HWND hDockHost, DHLeafNode* pNode, HDC hDC/*, const RECT* prcPaint // TODO: Use this to optimize.*/ )
{
	HWND hWnd = DockHost_GetNodeWindow( CONV(pNode) );
	RECT rNode;
	DockHost_GetNodeRect( hDockHost, CONV(pNode), &rNode );
	if( hWnd == NULL )
	{
		...
		if( !IsRectEmpty( &rIntersection ) )
			FillRect( hDC, &rIntersection, (HBRUSH)( COLOR_BTNFACE) );
	}
	else 
        if( 0 == (DockHost_GetNodeStyle( CONV(pNode) ) & DHNS_NO_TEAROFF) ) 
	    {
		    BOOL bCloseHot = DockHost_GetHotCloseButton().hNode == CONV(pNode);
		    // TODO: Pass prcPaint to DrawDockedCaption().
		    ::DrawDockedCaption( hWnd, hDC, rNode, TRUE, bCloseHot,(DockHost_GetNodeStyle( CONV(pNode) ) & DHNS_NO_CLOSE) == 0);
	    }
...
VOID PaintDividerNode( HWND hDockHost, DHDividerNode* pNode, HDC hDC )
{
    ...
    FillRect( hDC, &rDivider, (HBRUSH)(COLOR_BTNFACE) );
	//FillRect( hDC, &rDivider, InterpolateCOLORREF( GetSysColor( COLOR_3DFACE ),GetSysColor( COLOR_MENUHILIGHT ), 0.25f ) );
	...
	case WM_PAINT:
		{
			...
			if( DockHost_IsEmpty( hWnd ) )
				FillRect( hdc, &ps.rcPaint, (HBRUSH)(COLOR_BTNFACE) );
			else
			{
...
// Initialize the DockHost window class. Needed before any DockHost windows are created.
// ...
